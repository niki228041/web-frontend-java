      ...
      <div key={data.id} className=' bg-[#5c5c5c] rounded-xl flex flex-col items-center'>
          ...
          <p className=' font-normal p-2 text-xs'>{data.description}</p>
          {
            ...
            <div className="flex place-content-between w-full pl-2 pr-2">
                <button onClick={()=>{deleteCategory(data.id)}} className=' outline-2 w-[80px] outline-red-300 outline rounded-sm pr-3 pl-3 text-sx ml-1 mt-3 mb-5 hover:bg-red-500 hover:text-white hover:font-bold duration-150 hover:pl-4 hover:pr-4 '>Delete</button>
                
                <button onClick={()=>{navigate("/edit-category/" + data.id)}} className=' outline-2 w-[80px] outline-orange-300 outline rounded-sm pr-3 pl-3 text-sx mt-3 mb-5 hover:bg-orange-500 hover:text-white hover:font-bold duration-150 hover:pl-4 hover:pr-4 '>Edit</button>
            </div>
            )
            :
